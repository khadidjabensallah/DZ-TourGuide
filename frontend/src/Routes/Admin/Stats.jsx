        ...
        const fetchData = async () => {
            try {
                ...
                if (!adminId) {
                    ...
                    setError("Admin ID missing. Please contact support or re-login.");
                    setLoading(false);
                    return;
                }

                // Fetch all users to calculate stats
                const usersResponse = await AdminAPI.getAllUsers(adminId);
                const pendingResponse = await AdminAPI.getPendingGuides(adminId);

                if (usersResponse.success) {
                    const users = usersResponse.users;
                    const guides = users.filter(u => u.type === 'guide');
                    const tourists = users.filter(u => u.type === 'tourist');
                    const activeGuidesList = guides.filter(g => g.isActive); // Assuming isActive logic
                    ...
                    setStats(prev => ({
                        ...
                        activeGuides: activeGuidesList.length,
                        pendingGuides: pendingResponse.success ? pendingResponse.pending_guides.length : 0
                    }));

                    // Format guides for the table
                    const formattedGuides = guides.map(g => ({
                        id: g.id,
                        name: g.name,
                        region: "Algiers", // Placeholder as region isn't in user list API yet
                        standardTours: 0, // Placeholder
                        customTours: 0, // Placeholder
                        totalTours: g.stats?.tours_count || 0,
                        revenue: 0, // Placeholder
                        rating: g.stats?.rating || 0,
                        hasPhoto: !!g.photo_url
                    }));
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100">
            ...
            <main className="max-w-7xl mx-auto px-6 py-8">
                {/* Error Message */}
                {error && (
                    <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl mb-8 flex items-center gap-3 animate-fade-in">
                ...
                <div className="grid grid-cols-4 gap-4 mb-8">
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        ...
                        <div className="flex items-center gap-4 text-xs">
                            <span className="text-blue-600">{stats.touristsCount} Tourists</span>
                            <span className="text-purple-600">{stats.guidesCount} Guides</span>
                        </div>
                    </div>

                    <div className="bg-white rounded-xl shadow-sm p-6">
                        <p className="text-sm text-gray-600 mb-2">Active Guides</p>
                        <p className="text-3xl font-bold text-gray-900 mb-2">{stats.activeGuides}</p>
                        <div className="flex items-center gap-4 text-xs">
                            <span className="text-blue-600">{stats.activeGuides} Active</span>
                            <span className="text-purple-600">{stats.guidesCount - stats.activeGuides} Inactive</span>
                        </div>
                    </div>
